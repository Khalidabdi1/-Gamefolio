...
export function LibraryScreen({ backlog = false }: { backlog?: boolean }) {
  ...
  const selected = games.filter(
    (g) => s.entries[g.id] && (s.genre === "All genres" || g.genre === s.genre),
  );
  const grouped = statuses
    .filter((x) => s.filters.includes(x.id))
    .map((status) => ({
      ...status,
  ...
  return (
    ...
    >
      ...
      {backlog ? (
          ...
          {queued.map((g) => (
            <GameRow
              ...
              onComplete={() =>
                s.update(g.id, {
                  status:
                    s.entries[g.id].status === "completed"
                      ? "backlog"
                      : "completed",
                  progress: s.entries[g.id].status === "completed" ? 0 : 100,
                })
              }
            />
          ))}
        ...
        <View style={{ gap: 40 }}>
          {grouped.map((section) => (
            <View key={section.id} className="px-5">
              <Text
                className="mb-3 text-ink font-semibold"
                style={{ fontSize: 24, letterSpacing: 0.1 }}
              >
                {section.label}
              </Text>
              {s.view === "grid" ? (
                ...
                <View className="gap-2">
                  {section.games.map((g) => (
                    <GameRow
                      ...
                      compact={s.view === "list"}
                      onComplete={() =>
                        s.update(g.id, { status: "completed", progress: 100 })
                      }
                    />
                  ))}
                </View>
  ...
  return (
    <View className="items-center gap-3 px-8 py-20">
      ...
      <Text className="text-center text-muted">
        Change your filters or search for a game to add to your collection.
      </Text>
    </View>
  );
}
